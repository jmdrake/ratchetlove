...
//        echo $this->db->last_query();
		...
		{
			$fetch  = $result->row_array();
			$values = array('userid' => $fetch['id'], 'username' => $fetch['username']);
//echo $values;die;
...
//    }
    ...
    {
        $result= $this->db->query('SELECT *,ratchet_messages.id as msg_id
FROM ratchet_messages
INNER JOIN ratchet_register
ON ratchet_messages.from_id=ratchet_register.id
INNER JOIN ratchet_profile
ON ratchet_profile.uid=ratchet_register.id
WHERE  ratchet_messages.to_id='.$id);
//        $result = $this->db->get();
    ...
    function send_message_view($id)
    {
        $result= $this->db->query('SELECT *,ratchet_messages.id as msg_id
FROM ratchet_messages
INNER JOIN ratchet_register
ON ratchet_messages.to_id=ratchet_register.id
INNER JOIN ratchet_profile
ON ratchet_profile.uid=ratchet_register.id
WHERE  ratchet_messages.to_id='.$id);
//        $result = $this->db->get();
...
INNER JOIN ratchet_profile
ON ratchet_profile.uid=ratchet_register.id
WHERE ratchet_date_night.status=0 AND  ratchet_date_night.uid='.$id);
//        $result = $this->db->get();
        $fetch=$result->result_array();
        //echo $this->db->last_query();
        return $fetch;
...
INNER JOIN ratchet_profile
ON ratchet_profile.uid=ratchet_register.id
WHERE ratchet_date_night.accept=1 AND  ratchet_date_night.pid='.$id);
//        $result = $this->db->get();
        $fetch=$result->result_array();
        //echo $this->db->last_query();
        return $fetch;
